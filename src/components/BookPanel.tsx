...
import type { BookChallenge, BookManifest, BookNavState, BreadcrumbEntry } from '../types'
import {
...
function renderMarkdown(md: string): string {
  ...
  text = text.replace(/\x00I(\d+)\x00/g, (_, i: string) => {
    ...
    return `<code class="font-mono text-xs bg-slate-900 text-amber-300 px-1 rounded">${escHtml(code)}</code>`
  })
...
export function BookPanel({ navState, onNavStateChange, onEnterChallenge, onClose }: Props) {
  ...
  // Load guide when challenge is active
  useEffect(() => {
    ...
    // Check if this challenge already has a saved filesystem
  ...
  // Breadcrumb items: root name + intermediate levels
  const crumbItems = [
    ...
    <div className="flex flex-col h-full overflow-hidden text-xs select-none">
      ...
      <div className="bg-slate-900 py-1.5 px-2 border-b border-slate-700 flex-shrink-0 flex items-center gap-1">
        ...
        {navState.activeChallengeId && (
          <>
            <button onClick={() => handlePrevNext(-1)} disabled={!prevChallenge}
            ...
            <button onClick={() => handlePrevNext(1)} disabled={!nextChallenge}
            ...
            <div className="relative flex-shrink-0">
              <button onClick={() => setShowDotMenu(o => !o)}
              ...
              {showDotMenu && (
                <div className="absolute right-0 top-full mt-1 z-50 bg-slate-800 border border-slate-600 rounded shadow-xl py-1 min-w-[120px]"
                  ...
                  <button onClick={handleReset} disabled={!challengeHasFs}
        ...
        {/* Close book */}
        <button onClick={onClose} title="Close book"
              ...
              return (
                <button key={child.id}
                  ...
                  <svg className="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                    style={{ color: isExample ? '#94a3b8' : '#34d399' }}>
