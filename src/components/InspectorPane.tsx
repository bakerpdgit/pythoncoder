  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col rounded-lg border border-slate-700 bg-slate-900/60">
      <div className="border-b border-slate-700 px-3 py-2">
        <div className="text-[11px] font-bold uppercase tracking-wider text-slate-400">{title}</div>
        <div className="mt-1 text-xs text-slate-500">{root.label}</div>
      </div>
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto p-3">
        <div className="flex flex-wrap items-center gap-2 text-xs">
          ...
          {path.length > 0 && (
            <button onClick={() => setPath([])} className="ml-auto rounded border border-slate-600 px-2 py-1 text-slate-300 hover:border-sky-500">
              Reset Focus
            </button>
          )}
        </div>
        ...
        <div className="rounded-lg border border-slate-700 bg-black/20 p-3">
          ...
          <div className="mt-2 flex flex-wrap items-center justify-between gap-2">
            ...
            {isInspectorCompound(activeNode) && <span className="text-xs text-slate-400">{children.length} visible children</span>}
          </div>
          ...
          children.length > 0 ? (
            <div className="grid grid-cols-1 gap-2 xl:grid-cols-2">
              {children.map((child, index) => {
                ...
                if (canOpen) {
                  return (
                    <button key={`${child.label}-${index}`} onClick={() => setPath([...path, { index, label: child.label }])} className={cardClasses}>
            ...
            <div className="text-sm italic text-slate-500">No inspectable values are available at this scope.</div>
          )
        ) : null}
      </div>
    </div>
