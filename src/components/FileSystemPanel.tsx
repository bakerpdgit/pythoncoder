...
interface Props {
  ...
  // ── Teacher edit mode ──
  editMode?: boolean
  challengePyName?: string
  fileVisibility?: Record<string, boolean>
  onToggleFileVisibility?: (name: string) => void
}
...
export function FileSystemPanel({
  ...
  reloadTrigger,
  editMode, challengePyName, fileVisibility, onToggleFileVisibility,
}: Props) {
              ...
              <div key={entry.id}
                ...
                {renameId === entry.id ? (
                  ...
                  <>
                    ...
                    {editMode && entry.type === 'file' && entry.name !== challengePyName && (() => {
                      const visible = fileVisibility?.[entry.name] ?? true
                      return (
                        <button type="button"
                          onClick={e => { e.stopPropagation(); onToggleFileVisibility?.(entry.name) }}
                          title={visible ? 'Visible to students — click to hide' : 'Hidden from students — click to show'}
                          className={`flex-shrink-0 transition-colors ${visible ? 'text-emerald-400 hover:text-emerald-300' : 'text-slate-500 hover:text-slate-300'}`}>
                          {visible ? (
                            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
